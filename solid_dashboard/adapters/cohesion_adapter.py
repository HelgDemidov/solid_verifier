...
class ClassInfo:
    ...
    # заполняется функцией _classify_class на этапе _build_class_info
...
class CohesionAdapter(IAnalyzer):
    ...
    def _build_class_info(self, class_node: ast.ClassDef, file_path: Path) -> ClassInfo:
        ...
        # определяем семантический тип класса по AST — модульная функция, не метод адаптера
        class_info.kind = _classify_class(class_node)
...
# ================================
# КЛАССИФИКАЦИЯ ТИПА КЛАССА
# ================================

def _classify_class(class_node: ast.ClassDef) -> str:
    """
    Определяет семантический тип класса по AST-узлу.

    Чистая функция уровня модуля: не использует состояние адаптера,
    оперирует только переданным class_node. Вынесена из CohesionAdapter
    для устранения нарушения SRP и обеспечения прямой тестируемости.
